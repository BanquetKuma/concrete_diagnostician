// Individual question screen with answer functionality
...
export default function QuestionScreen() {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      if (!year || !questionId) return;

      try {
        setIsLoading(true);
        setError(null);
        const response = await apiClient.getQuestion(parseInt(year), questionId);
  ...
  const handleNextQuestion = () => {
    ...
    if (nextQuestionId && year) {
      router.push({
        pathname: '/questions/[year]/[questionId]',
        params: { year: year as string, questionId: nextQuestionId }
      });
    } else {
  ...
  const handlePreviousQuestion = () => {
    ...
    if (prevQuestionId && year) {
      router.push({
        pathname: '/questions/[year]/[questionId]',
        params: { year: year as string, questionId: prevQuestionId }
      });
    }
  };
