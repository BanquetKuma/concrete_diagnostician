...
### EAS Build / Submit (App Store)

- Production ビルド + 自動サブミット: `eas build --profile production --platform ios --auto-submit`
- **version インクリメント必須**: App Store Connect に submit する際、`app.json` の `expo.version` が前回提出済みバージョンと同じだと `You've already submitted this version` エラーで失敗する。新しいビルドを submit する前に必ず `expo.version` をインクリメントすること（例: `1.0.0` → `1.0.1`）。`ios.buildNumber` は `eas.json` の `autoIncrement: true` で自動インクリメントされるが、`version` は手動更新が必要。
...
外出先や実家など、ローカル開発環境がない場所からでもGitHub Codespacesを使用して開発が可能。
...
- **Codespacesでできること**:
  - コード編集・実装
  - Claude Codeの使用（`claude`コマンド）
  - Expo Webでの確認（`npm run web`）
  - lint、型チェック等の開発ツール実行
...
#### Codespacesの制限事項（モバイル実機テスト）

**重要**: Codespacesからのモバイル実機テストは現時点で動作しません。

- **ngrokトンネルの問題**:
  - `npm run start:tunnel`を実行すると「ngrok tunnel took too long to connect」エラーが発生
  - Codespaces環境とngrokの相性問題により、トンネル接続がタイムアウトする

- **ポートフォワーディングの問題**:
  - Codespacesのポートを公開（public）に設定しても、Expo Goから接続できない
  - 「Could not connect to development server」エラーが発生
...
- **推奨する使い分け**:
  ...
  | **Windows PowerShell** | モバイル実機テスト（同一WiFiでQRスキャン） |
